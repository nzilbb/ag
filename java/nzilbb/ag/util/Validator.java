	    ...
	    for (Layer childLayer : layer.getChildren().values())
	    {
		  ...
		  { // out of order
			...
			for (Anchor other : anchorsToReset)
			{
			   ...
			   other.setConfidence(Constants.CONFIDENCE_NONE);
			}			
		     }
		  ...
		  { // overlap
			...
			{ // we can narrow the child to directly follow the last child
			   if (childLayer.getSaturated())
			   {
				 ...
				 lastAnchoredChild.getEnd().setOffset(dMidPoint));
			      lastAnchoredChild.getEnd().setConfidence(Constants.CONFIDENCE_DEFAULT);
			      if (child.getEnd().getOffset() == null 
				  || child.getEnd().getOffset() <= child.getStart().getOffset())
			      {
				 changes.addAll( // record changes generated by:
				    child.getEnd().setOffset(dLastOffset));
				 child.getEnd().setConfidence(Constants.CONFIDENCE_DEFAULT);
			      }
			   }
			   else
			   {
				 ...
				 child.getStart().setOffset(dMidPoint));
			      child.getStart().setConfidence(Constants.CONFIDENCE_DEFAULT);
			   }
			   if (child.getEnd().getOffset() == null 
			       || child.getEnd().getOffset() <= child.getStart().getOffset())
			   {
			...
			{ // can't narrow both
			   ...
			   { // we can narrow the child to directly follow the last child
				 ...
				 child.getStart().setConfidence(Constants.CONFIDENCE_DEFAULT); // TODO should the status be copied too? should it be getDeltaOffset or getOffset
			      }
			   }
			   else
			   {
				 ...
				 else
				 {
				    ...
				    lastAnchoredChild.getEnd().setConfidence(Constants.CONFIDENCE_DEFAULT); // TODO copy the status too?  revert to getOffset?
				 }
			      }
			      else
			      {
