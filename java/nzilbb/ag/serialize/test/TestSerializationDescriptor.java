...
package nzilbb.ag.serialize.test;
...
{      
   ...
   @Test public void toJson() {
      ...
      assertEquals("Name", json.get("name").toString());
      assertEquals("0.0.0", json.get("version").toString());
      assertEquals("text/plain", json.get("mimeType").toString());
      assertEquals("00000000.0000", json.get("minimumApiVersion").toString());
      assertEquals(2, json.getInt("numberOfInputs"));
      assertEquals(".txt", json.getJsonArray("fileSuffixes").get(0).toString());
   }

   public static void main(String args[]) {
      org.junit.runner.JUnitCore.main("nzilbb.ag.serialize.test.TestSerializationDescriptor");
   }
}
