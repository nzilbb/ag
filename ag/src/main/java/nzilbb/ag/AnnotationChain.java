...
public class AnnotationChain extends LinkedHashSet<Annotation> implements SortedSet<Annotation> {
    ...
    while (itStartCandidates.hasNext()) {
      ...
      while(bFound) {	
        ...
        for (Annotation instant : instants) {
          if (instant.getEnd().equals(last.getStart())) {
            chain.add(instant);
          }
        }
   ...
   */
  protected void findChain(
    Anchor start, double endOffset, Set<String> excludeLayers, Set<Anchor> excludeAnchors) {
    ...
    for (Annotation startsHere : start.getStartingAnnotations())
    {
      ...
      excludeAnchors.add(startsHere.getEnd());
      AnnotationChain nextPart = new AnnotationChain(
        startsHere.getEnd(), endOffset, excludeLayers, excludeAnchors);
      if (nextPart.size() > 0) {
        // prefix any instants that are at the start
        for (Annotation a : start.getStartingAnnotations()) {
          if (a.getChange() == Change.Operation.Destroy) continue;
      ...
      if (startsHere.getEnd().getOffset() != null
          && startsHere.getEnd().getOffset() <= endOffset) { // this is the last in the chain
        // prefix any instants that are at the start
        for (Annotation a : start.getStartingAnnotations()) {
          if (a.getChange() == Change.Operation.Destroy) continue;
          if (a.getInstantaneous()) {
   ...
   */
  protected AnnotationChain(
    double startOffset, Anchor end, Set<String> excludeLayers, Set<Anchor> excludeAnchors) {
    if (end != null) {
  ...
  protected void findChain(
    ...
    for (Annotation endsHere : end.getEndingAnnotations()) {
      ...
      AnnotationChain previousPart = new AnnotationChain(
        startOffset, endsHere.getStart(), excludeLayers, excludeAnchors);
      ...
      if (endsHere.getStart().getOffset() != null
          && endsHere.getStart().getOffset() >= startOffset) { // this is the first in the chain
        // prefix any instants that are at the start
  ...
  public Annotation first() throws NoSuchElementException {
    return iterator().next();
  }

  public Annotation last() throws NoSuchElementException {
