...
public class Annotation extends TrackedMap implements Comparable<Annotation> {
  ...
  public Annotation setParent(Annotation newParent, boolean append) {
    ...
    if (newParent == null) {
      ...
      if (!newParent.getAnnotations().containsKey(getLayerId())) { // ensure the collection exists
...
*/
   ...
   */
  public Annotation(
    String id, String label, String layerId, String startId, String endId, String parentId) {
    setId(id);
    setLabel(label);
    setLayerId(layerId);
    setStartId(startId);
  ...
  public Annotation(
    String id, String label, String layerId, String startId, String endId, String parentId,
    int ordinal, Integer confidence) {
    setId(id);
    setLabel(label);
    setLayerId(layerId);
    setStartId(startId);
  ...
  public Annotation first(final String layerId) { // TODO first simply look for a start.startOf(laterId) annotation?
    ...
    if (getGraph() != null) {
      ...
      if (commonAncestorLayer == graph.getSchema().getRoot()) { // related only temporally
      ...
      { // we are an ancestor of the target layer
        ...
        } else { // layerId != highestAlignedLayerId
  ...
  public Annotation[] annotations(String layerId) {
    ...
    } catch(NullPointerException exception) { // the layer doesn't exist
      return null;
    }
  } // end of annotations()
  ...
  */
  ...
  public Annotation createTag(String layerId, String label) {
    ...
    if (getGraph().getLayer(tag.getLayerId()).getParent() == getLayer()) { // tag is child of this
      tag.setParent(this);
