 ...
 */
public class FfmpegCensor implements MediaCensor {
   ...
   *      from 0.0-1.0 will be uncensored, from 1.0-2.0 will be obfuscated, and from 2.0
   *      onward will be uncensored.</li>  
   *  <li>If <var>boundaries</var> contains three points, at 0.0, 1.0, and 2.0, then the
   *      media from 0.0-1.0 will be obfuscated, from 1.0-2.0 will be uncensored, and
   *      from 2.0 onward will be obfuscated.</li>  
   * @param sourceType The MIME type of the source media.
   * @param source The (old) original file.
   * @param boundaries A list of time points, in seconds, which define the boundaries of
  ...
  public MediaThread start(
    String sourceType, File source, List<Double> boundaries, File destination)
    throws MediaException {
    if (getFfmpeg() == null) throw new MediaException("ffmpeg location not specified.");
    if (!getFfmpeg().exists())
      throw new MediaException("ffmpeg program doesn exist: " + getFfmpeg().getPath());

    // remove identical pairs from boundarie list
    Double previousBoundary = null;
    ListIterator<Double> i = boundaries.listIterator();
    while (i.hasNext()) {
      Double boundary = i.next();
      if (boundary == null) {
        // eliminate nulls while we're here
        i.remove();
      } else {
        if (previousBoundary != null) {
          if (boundary.equals(previousBoundary)) {
            ...
            // reset previousBoundary
            if (i.hasPrevious()) {
              previousBoundary = i.previous();
            } else {
              previousBoundary = null;
            }
          } // equal pair
        } // there is a last boundary to compare with
      } // not null boundary
    ...
    if (boundaries.size() == 0) { // no points, so just copy the source to the destination
