 ...
 * to determine the minimum edit path (or distance) between two sequences. While
 * traditionally this is between two strings (sequences of characters), the implementation
 * uses templates to support sequences of any type, and interfaces for comparators, to
...
public class MinimumEditPath<T> {
  ...
  /**
   * Getter for {@link #comparator}: The comparator used when determining the distance
   * between two sequence elements. 
  ...
  /**
   * Setter for {@link #comparator}: The comparator used when determining the distance
   * between two sequence elements. 
   * @param Newcomparator The comparator used when determining the distance between two
   * sequence elements. 
   */
   ...
   * <p>This implementation, when faced with a change with the same distance as a delete
   * or insert,  will favor the delete or insert over change. 
   * This is so you can favor delete/insert if desired, by simply upping the change
   * distance to 2, i.e. with: 
   ...
   */
  protected EditStep<T> minimumEdit(
    EditStep<T> left, EditStep<T> below, EditStep<T> diagonal, T from, T to) {
