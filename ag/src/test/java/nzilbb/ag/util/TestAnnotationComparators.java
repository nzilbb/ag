...
public class TestAnnotationComparators
{
  ...
  @Test public void byAnchorShortestFirst() 
  {
    Graph g = new Graph();
    g.setId("my graph");
    g.setCorpus("cc");
    ...
    Iterator<Annotation> order = annotations.iterator();
    assertEquals("earliest first", "turn1", order.next().getId());
    assertEquals("end order - phone before word", "phone1", order.next().getId());
    assertEquals("layerId order - pos before word", "pos1", order.next().getId());
    assertEquals("layerId order - word after pos", "word1", order.next().getId());
    assertEquals("end order - phrase after first word", "phrase2", order.next().getId());
    assertEquals("phone2", order.next().getId());
    assertEquals("zero-length between previous and next", "zerolength", order.next().getId());
    assertEquals("end order - phone before word", "phone3", order.next().getId());
    assertEquals("layerId order - pos before word", "pos2", order.next().getId());
    assertEquals("layerId order - word after pos", "word2", order.next().getId());
    assertEquals("end order - phrase after first word", "phrase1", order.next().getId());
    assertEquals("phone4", order.next().getId());
    assertEquals("phone5", order.next().getId());
    assertEquals("phone6", order.next().getId());
    assertEquals("word3", order.next().getId());
  ...
  @Test public void byDistance() 
  {
    ...
    AnnotationComparatorByDistance comparator
      = new AnnotationComparatorByDistance(g.getAnnotation("word1"));
    TreeSet<Annotation> annotations = new TreeSet<Annotation>(comparator);
    annotations.add(g.getAnnotation("phone1"));
    annotations.add(g.getAnnotation("phone2"));
    annotations.add(g.getAnnotation("word1"));
    annotations.add(g.getAnnotation("word2"));
    annotations.add(g.getAnnotation("word4"));
    annotations.add(g.getAnnotation("turn1"));
    Iterator<Annotation> order = annotations.iterator();
    assertEquals("unanchored first: "+annotations, "word4", order.next().getId());
    assertEquals("full overlap - earliest first: "+annotations, "turn1", order.next().getId());
    assertEquals("full overlap - self: "+annotations, "word1", order.next().getId());
    assertEquals("partial overlap - earliest first: "+annotations, "phone1", order.next().getId());
    assertEquals("partial overlap - latest last: "+annotations, "phone2", order.next().getId());
    assertEquals("adjacent: "+annotations, "word2", order.next().getId());
    assertFalse(order.hasNext());
  }

  public static void main(String args[]) 
  {
    org.junit.runner.JUnitCore.main("nzilbb.ag.util.TestAnnotationComparators");
  }
}
