...
public class CsvToJson extends CommandLineProgram
{
   ...
   public void start()
   {
	 ...
	 new Layer("pronounce", "pronounce", 0, false, false, true, "transcript", true),
	    ...
	    try
	    {
		  ...
		  System.out.println(warning);
	       ...
	       // convert to JSON
               ...
               final Vector<NamedStream> s = new Vector<NamedStream>();
               toJson.serialize(Utility.OneGraphSpliterator(g),
                                g.getSchema().getLayers().keySet().toArray(new String[0]),
                                stream -> s.add(stream),
                                warning -> System.out.println(warning),
                                exception -> exceptions.add(exception));
	       s.elementAt(0).save(f.getParentFile());	    
	    }
