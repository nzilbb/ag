...
public class GraphStoreHarness implements GraphStore {
  File dir;
  HashMap<String,File> savedMedia = new HashMap<String,File>();

  /**
   * Constructor.
   */
  public GraphStoreHarness(File dir) {
    this.dir = dir;
  } // end of constructor
  ...
  public void saveMedia(String id, String trackSuffix, String mediaUrl)
    ...
    try {
      File originalFile = new File(new URI(mediaUrl));
      File savedFile = File.createTempFile(id + ".", "."+originalFile.getName());
  ...
  public MediaFile[] getAvailableMedia(String id) 
    throws StoreException, PermissionException, GraphNotFoundException {
    return savedMedia.values().stream()
      .map(file -> new MediaFile(file))
      .collect(Collectors.toList()).toArray(new MediaFile[0]);
  }
