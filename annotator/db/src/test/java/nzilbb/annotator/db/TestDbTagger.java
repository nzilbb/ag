...
public class TestDbTagger {
  ...
  static DbTagger annotator = new DbTagger();
  ...
  public static void install() throws Exception {
    ...
    System.out.println("Installing table...");
    ...
    System.out.println("Table installed.");
  }
	 
  public static File dir() throws Exception { 
    URL urlThisClass = TestDbTagger.class.getResource(
      TestDbTagger.class.getSimpleName() + ".class");
  ...
  @Test public void setInvalidTaskParameters() throws Exception {
    ...
    try {
      ...
      fail("Should fail with nonexistent table");
    } catch (InvalidConfigurationException x) {
    }
    try {
      annotator.setTaskParameters(
  ...
  /** Test that tables and corresponding dictionaries can be added and removed. */
  @Test public void tableManagement() throws Exception {

    // ensure there's no leftover version of the extra table
    String error = annotator.deleteTable("dict");
    ...
    DbDictionary table = (DbDictionary)dictionary;
    // TODO case sensitivity not supported with Derby
    // table.setExactMatch(true);
    ...
    table.setExactMatch(true);
    assertTrue("MySQL for case-sensitive:" + table.rawQuery,
               table.rawQuery.endsWith("WHERE \"type\" = BINARY ? ORDER BY serial"));
    // Can't support case-sensitivity for Derby as well as MySQL, so we prioritise MySQL
    assertTrue("Derby SQL for case-sensitive: " + table.translatedQuery,
               table.translatedQuery.endsWith("WHERE \"type\" = ? ORDER BY serial"));
    table.setExactMatch(false);
    assertTrue("MySQL for case-insensitive: " + table.rawQuery,
               table.rawQuery.endsWith("WHERE \"type\" = ? ORDER BY serial"));
    assertTrue("Derby SQL for case-insensitive: " + table.translatedQuery,
               table.translatedQuery.endsWith("WHERE \"type\" = ? ORDER BY serial"));
    ...
    assertEquals("Correct number of fields in table: " + fields,
                 2, fields.size());
    ...
    // can remove tables
    assertEquals("Can delete table", "", annotator.deleteTable("dict"));
    ids = annotator.getDictionaryIds();
  ...
  @Test public void tableLink() {
    ...
    try {
      ...
      assertEquals("Ensure tableLink is set",
                   "http://example.com/edit/annotator/ext/DbTagger/entry.html?l={0}&f={1}&e={2}",
                   annotator.getTableLink());
      assertEquals("tag layer type correct",
  ...
  public static void main(String args[]) {
    org.junit.runner.JUnitCore.main("nzilbb.annotator.db.TestDbTagger");
  }
}
