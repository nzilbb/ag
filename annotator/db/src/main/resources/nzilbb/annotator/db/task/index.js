  ...
  addLayerOptions(
    ...
    layer => layer.parentId == schema.wordLayerId
      || (layer.parentId == schema.turnLayerId
          && layer.alignment == 2
          && ![schema.wordLayerId, schema.utteranceLayerId].includes(layer.id)));
    ...
    if (running == "true") {
      ...
      loadDictionaryOptions(() => {
        ...
        if (dictionary.selectedIndex <= 0) {
          // select the first dictionary for that table
          var table = document.getElementById("table").value;
          for (var d in dictionary.options) {
            if (dictionary.options[d].value.startsWith(table)) {
...
/** Automatically set the tableLink setting to work for editing the entry in LaBB-CAT */
function inferLink() {
  // turn something like:
  // http://example.com/labbcat/admin/annotator/task/DbTagger/?places
  // ...into something like:
  // http://example.com/labbcat/annotator/ext/DbTagger/entry.html?t={0}&f={1}&e={2}
  document.getElementById("tableLink").value
    = window.location.toString()
    .replace("/admin/","/")
    .replace("/task/","/ext/")
    .replace(/\?.*$/,"")
    +"entry.html?t={0}&f={1}&e={2}";
  return false;
}
